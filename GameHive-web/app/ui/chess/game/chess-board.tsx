...
import { isValidPromotionContext, IPromotionContext } from '@/app/lib/promotions';
...
function handleDragStart(e: any, boardState: IBoardState, setDraggingFromSquare: any): void {
  setDraggingFromSquare(e.active.id);
}

function handleDragEnd(e: any, boardState: IBoardState, setDraggingFromSquare: any, setPromotionContext: any): void {
  if (e.over) {
...
function handlePromotionSelection(promotionPiece: TPiece, promotionContext: IPromotionContext, boardState: IBoardState, setPromotionContext: any): void {
  if (isValidPromotionContext(promotionContext, boardState)) {
    boardState.makeMove({from: promotionContext.from, to: promotionContext.to, promotionPiece});
...
export default function ChessBoard({boardState}: {boardState: IBoardState}) {
  ...
  const [promotionContext, setPromotionContext] = useState({from: -1, to: -1} as IPromotionContext);
  ...
  return (
    <DndContext 
      ...
      onDragEnd={(e) => handleDragEnd(e,boardState,setDraggingFromSquare,setPromotionContext)} 
            ...
            {files.map(file => {
              ...
              return (
                <ChessSquare key={`${sq}`}
                  ...
                  onPromotionSelection={(promotionPiece: TPiece) => handlePromotionSelection(promotionPiece, promotionContext, boardState, setPromotionContext)}
                />
              );
            })}
          </div>
