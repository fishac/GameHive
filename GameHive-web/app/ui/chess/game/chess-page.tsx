...
function handleChessEngineWorkerResponse(
  ...
  if (response.responseType === CHESS_ENGINE_WORKER_MESSAGE_TYPES.INITIALIZE) {
    ...
    response.responseType === CHESS_ENGINE_WORKER_MESSAGE_TYPES.SUGGEST_MOVE
  ) {
  ...
  useEffect(() => {
    if (chessEngineWorker) {
      chessEngineWorker.onmessage = (e: MessageEvent) =>
        handleChessEngineWorkerResponse(
          e,
          humanPlayerColor,
          chessEngineWorker,
          boardState,
  ...
  return (
    <div className="flex flex-row justify-center mt-8">
      ...
      <div className="flex flex-col justify-between w-72 bg-blue-500">
        <Timer baseTimeMillis={180000} incrementMillis={0} />
        <Timer baseTimeMillis={180000} incrementMillis={0} />
      </div>
    </div>
  );
